...
public abstract class GEUtils extends LoopScript {
    ...
    public static void openGEIfNotOpen() {
    ...
    public static void buyItemsFromShoppingList(List<String> items, List<Integer> quantities, int price) {
        int i = 0;
        for(String item : items) {
            makeBuyOffer(price, item, quantities.get(i));
            stx.grandExchange().confirmOffer();
            i += 1;
        }
    }
    ...
    public static boolean saleComplete(String item) {
        ...
        for(GrandExchangeSlot slot : slots) {
            WidgetChild slotWidget = stx.widgets().get(465).getChild(slot.getIndex() + 7).getChild(19);
            System.out.println(slotWidget.getText());
    ...
    public static void makeBuyOffer(String item, int quantity, int priceIncreases) {
    ...
    public static void makeBuyOffer(int price, String item, int quantity) {
        stx.grandExchange().newBuyOffer(item);
    ...
    public static void makeSellOffer(String item, int quantity) {
        stx.grandExchange().newSellOffer(item);
        stx.grandExchange().setQuantity(quantity);
        stx.grandExchange().confirmOffer();
    }

    public static void makeSellOffer(String item) {
        stx.grandExchange().newSellOffer(item);
        stx.grandExchange().confirmOffer();
    }
    ...
    public static boolean buyOfferCompleted(GrandExchangeSlot slot) {
...
//    }
    ...
    public static boolean sellOfferCompleted(GrandExchangeSlot slot) {
